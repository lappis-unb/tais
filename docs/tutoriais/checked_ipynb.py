...
def md_to_ipynb(notebook_files, md_files):
    ...
    for i, notebook_file in enumerate(notebook_files):
        if not os.path.isfile("notebooks/" + notebook_file):
            ...
            os.system("notedown " + md_files[i] +
                      " > notebooks/" + notebook_file)
            # Remove extra first line generated
            os.system("sed '1d' " + notebook_file +
                      " > tmpfile.md; mv tmpfile.md " + notebook_file)


if __name__ == "__main__":
    logger.info("Checking notebooks process started")
    md_files = get_md_files()
    notebook_files = [md_file.replace('md', 'ipynb')
